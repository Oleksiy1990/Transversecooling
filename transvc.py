...
def gauss_I(radius,waist,power):
    """
    1D normalized intensity of a Gaussian laser beam
    """
    intensity = power*(2/(np.pi*waist**2))*np.exp(-2*(radius**2)/waist**2)
    return intensity
...
def diffeqs(variables,t,params):
    ...
    x,vx,y,vy = variables # positions and velocities; x is along, y is transversal
    k_vector_laser,linewidth,detuning,waist,power = params

    s0 = gauss_I(x,waist,power)/blueIsat # sat. param is pos.-dependent

    derivs = [vx,0, \
    vy, (1/mSr84)*molasses_force(k_vector_laser,linewidth,s0,detuning,vy)]
    return derivs
